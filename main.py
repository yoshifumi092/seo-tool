...
【件数ルール】
- 問題性のある箇所はすべて抽出する（見落としは削除申請の材料を減らすため厳禁）
- 同趣旨の指摘は統合し、独立した論点は必ず分けて指摘する
- 最低10件以上を目指す（問題の多い記事では15〜25件になることも正常）
- 「もう十分だろう」と途中で止めることは禁止。最後の段落まで必ずスキャンを完了してから出力する
- D類型（商標名を使った不正誘導）は必ず最初に指摘する
- severity 1〜2の軽微な指摘も削除申請の補強材料になるため省略しない
