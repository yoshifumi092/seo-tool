...
async def analyze_with_claude(text: str, url: str, trademark: str = "") -> dict:
    ...
    prompt = f"""あなたは誹謗中傷・風評被害の削除申請を専門とする法務アシスタントです。
以下のWebページは、投資・副業・稼ぐ系ビジネス（スクール・コンサル・無形商材）の
商標・代表者・講師を攻撃するネガティブ記事です。
ホスティング会社やGoogleへの削除申請に直接使える証拠リストを作成してください。
{trademark_hint}
...
① 根拠のない断定的ネガティブ表現（最優先）
   例：「詐欺」「詐欺師」「悪質」「危険」「怪しい」「やばい」「ひどい」
       「騙された」「被害」「被害者」を証拠なく断定
       「稼げない」「効果ない」「意味ない」「返金できない」を断定

② 商標・代表者・講師名を使った信用毀損
   例：「○○（商標名）は詐欺」「○○代表は信用できない」
       「○○講師の実力はない」「○○は怪しい人物」

③ アフィリエイト・競合誘導（記事の裏の目的）
   例：対象を否定した後に「代わりにこちら→」と誘導
       「おすすめしない理由→おすすめはこれ」の構成
       報酬リンク付きで競合サービスを推薦

④ 匿名・出典不明情報を事実として断定
   例：「口コミによると〜」「〜という声が多い」「評判が悪い」
       「〜という被害報告がある」を根拠に断定

⑤ 購入・参加を妨げる営業妨害表現
   例：「やめたほうがいい」「絶対に手を出すな」「危険なので注意」
       「登録前に必ず読め」等のキャッチ+否定の構成

以下のJSON形式のみで返してください（前置き・説明・コードブロック不要）：

{{
  "article_title": "記事タイトル（原文）",
  "trademark": "攻撃対象の商標・サービス名・代表者名（最も重要な1つ）",
  "violations": [
    {{
      "text": "問題箇所の原文（10〜30文字。記事テキストから一字一句そのまま）",
      "type": "①根拠なき断定 または ②商標・個人名の信用毀損 または ③アフィリエイト誘導 または ④匿名情報の断定 または ⑤営業妨害",
      "explanation": "この記述が名誉毀損・信用毀損・業務妨害にあたる理由を2文で。削除申請書に転用できる法的表現で記載。"
    }}
  ]
}}

絶対ルール：
- violations は必ず10〜15件出力する
- "text" は記事テキストから10〜30文字を一字一句そのまま引用すること（要約・改変・追加禁止）
- textが短いほど検索で正確にヒットする。30文字を超えないこと
- ①のパターンを最優先で探し、なければ②〜⑤の順で探す
