...
1文目：〔問題となる行為・表現・構成〕は、〔問題の本質〕にあたる。
2文目：これは〔法令名〕第〔条番号〕（〔条名称〕）に該当する可能性がある。

出力例：
「対象の人名で流入した読者を記事冒頭から競合サービスへ誘導する構成は、商標名を利用した不正な集客誘導にあたる。これは不正競争防止法第2条第1項第21号（営業誹謗行為）に該当する可能性がある。」
「根拠を示さずに「詐欺まがい」と断定する記述は、対象の社会的評価を不当に低下させる表現にあたる。これは刑法第230条第1項（名誉毀損）に該当する可能性がある。」
...
{{
  ...
  "violations": [
    {{
      ...
      "deletion_comment": "〔問題となる行為・構成〕は、〔問題の本質〕にあたる。これは〔法令名〕第〔条番号〕（〔条名称〕）に該当する可能性がある。"
    }}
  ],
...
async def _call_claude_once(
    ...
    system_prompt = (
        ...
        "【最重要ルール】deletion_comment は必ず以下の2文構成で出力すること：\n"
        "1文目：〔問題となる行為・構成〕は、〔問題の本質〕にあたる。\n"
        "2文目：これは〔法令名〕第〔条番号〕（〔条名称〕）に該当する可能性がある。\n"
        "例：「対象の人名で流入した読者を競合サービスへ誘導する構成は、商標名を利用した不正な集客誘導にあたる。"
        "これは不正競争防止法第2条第1項第21号（営業誹謗行為）に該当する可能性がある。」\n"
