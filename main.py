...
async def analyze_with_claude(text: str, url: str, trademark: str = "") -> dict:
    ...
    prompt = f"""あなたは日本のSEOコンサルタント兼法務アドバイザーです。
ネガティブな記事を分析し、ホスティングサービスへの削除申請に使う証拠書類を作成してください。
{trademark_hint}
...
【特定すべき違反カテゴリ（優先度順）】
1. 名誉毀損 - 根拠なく個人・企業を傷つける記述、虚偽の事実の摘示
2. 営業妨害 - 業務を妨害する虚偽・誇大・根拠のない記述
3. 寄生マーケティング - 他社商標・ブランド名を利用して自社サービスへ誘導する記述
4. 不正競合 - 競合他社を不当に貶める、虚偽の比較・格付けをする記述
5. 著作権侵害 - 無断転載・引用超過の疑いがある記述
6. 虚偽・根拠なし - エビデンスのない主張、伝聞（「〜らしい」「〜だそうだ」等）
7. 印象操作 - 読者の判断を誤らせる誇張・バイアス・誘導表現
8. 過大なネガティブ表現 - 事実を大幅に超えた侮辱的・扇動的表現
...
{{
  ...
  "violations": [
    {{
      ...
      "type": "違反カテゴリ名",
      "explanation": "この記述が問題である具体的な理由（2〜3文。ホスティング担当者が削除を決断できるよう、明確かつ強く指摘してください）"
    }}
  ]
}}

重要なルール：
- violationsは最低5件、可能なら10〜20件、問題箇所をすべて網羅してください
- "text"は必ず記事中に実在するテキストをそのまま引用してください（変形・要約禁止）
- この記事はネガティブサイトであることを前提に、あらゆる問題点を積極的に指摘してください
...
このテキストの違反カテゴリと削除理由を特定してください。

テキスト：「{text[:500]}」

以下のJSON形式のみで回答してください：
{{
  "type": "違反カテゴリ（名誉毀損/営業妨害/寄生マーケティング/不正競合/著作権侵害/虚偽・根拠なし/印象操作/過大なネガティブ表現 のいずれか）",
  "explanation": "この記述が問題である具体的な理由（2〜3文）"
}}"""
