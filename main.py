...
deletion_comment は以下の形式で1〜2文で出力すること。条文テーブルに該当があれば条項番号を自然に組み込む。フルの条文テキストは絶対に引用しない。

形式：「〔問題表現の一部〕という記述は〔法令名〕第〔条番号〕（〔問題の性質〕）に該当すると考えられ、〔削除を求める理由を簡潔に〕。」

例：「「〇〇は詐欺まがいの手口」という表現は刑法第230条第1項（名誉毀損）に該当すると考えられ、根拠のない断定により対象の社会的評価を不当に低下させるものとして削除を求める。」
例：「商標名をタイトルに使用し別サービスへ誘導する構成は商標法第36条第1項に該当すると考えられ、商標権者の集客利益を不当に侵害するものとして削除を求める。」
例（条文テーブル該当なし）：「一方的なネガティブ情報のみを選別掲載する構成は読者に誤解を与えるものであり、削除を求める。」
...
{{
  ...
  "violations": [
    {{
      ...
      "deletion_comment": "「〔問題表現〕」という記述は〔法令名〕第〔条番号〕（〔問題の性質〕）に該当すると考えられ、〔削除理由〕。条文テーブル該当なしの場合は法令名を省き問題の本質のみで完結させる"
    }}
  ],
...
async def _call_claude_once(
    ...
    system_prompt = (
        ...
        "【最重要ルール】deletion_comment は以下の形式で出力すること（条文テーブル該当ありの場合）：\n"
        "「「〔問題表現〕」という記述は〔法令名〕第〔条番号〕（〔問題の性質〕）に該当すると考えられ、〔削除理由〕。」\n"
        "例：「「〇〇は詐欺まがいの手口」という表現は刑法第230条第1項（名誉毀損）に該当すると考えられ、"
        "根拠のない断定により対象の社会的評価を不当に低下させるものとして削除を求める。」\n"
        "フルの条文テキストは絶対に引用しない。条項番号だけを自然に文章に組み込む。\n"
        "legal_basis は「○○法第○条第○項（問題の性質）」という短い形式のみ。\n"
